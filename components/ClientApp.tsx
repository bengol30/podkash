...
function episodeSocialText(episode: Pick<Episode, 'socialText'>) {
  return episode.socialText?.trim() || '';
}
...
function useStore(initialStore?: Store) {
  ...
  const replaceStoreFromServer = (next: Partial<Store> | Store) => {
  ...
  return [store, setStore, replaceStoreFromServer] as const;
}
...
export function DashboardClient() {
  const [store, setStore, replaceStoreFromServer] = useStore();
  ...
  async function acknowledgeDashboardAudioSummary(job: MarketingAudioSyncJob) {
    ...
    if (storeRes?.ok) replaceStoreFromServer(await storeRes.json());
  }
  return <>
...
function shortDateTime(value?: string, iso?: string) {
  let d: Date | null = null;
  if (iso) { const x = new Date(iso); if (!Number.isNaN(x.getTime())) d = x; }
  if (d) return `${d.getDate()}.${d.getMonth() + 1} ${String(d.getHours()).padStart(2, '0')}:${String(d.getMinutes()).padStart(2, '0')}`;
  const raw = String(value || '').trim();
  if (!raw) return '—';
  const dm = raw.match(/(\d{1,2})\.(\d{1,2})/);
  const tm = raw.match(/(\d{1,2}):(\d{2})/);
  if (dm) return `${Number(dm[1])}.${Number(dm[2])}${tm ? ` ${tm[1].padStart(2, '0')}:${tm[2]}` : ''}`;
...
export function EpisodesClient() {
  ...
  const [ytUploading, setYtUploading] = useState<string | null>(null);
  ...
  async function uploadEpisodeToYoutube(e: Episode, isHost: boolean){
    const file = e.driveAssetStatus?.fullVideo?.files?.[0];
    if(!file?.id){ alert('אין קובץ וידאו מלא בתיקיית ה-Drive של הפרק. בצעו “סנכרון Drive” או העלו את הקובץ לתיקיית “הפרק המצולם המלא”, ואז נסו שוב.'); return; }
    if(!window.confirm(`להעלות את הפרק המלא “${e.title}” מ-Drive ישירות ל-YouTube? הסרטון יועלה כפרטי (אפשר לשנות/לפרסם אחר כך מאזור ניהול ה-YouTube).`)) return;
    const key = `${isHost ? e.ownerHostId : 'a'}-${e.id}`;
    setYtUploading(key);
    try{
      const description = [e.topic, e.host ? `מנחה: ${e.host}` : '', (e.guests && e.guests !== '—') ? `אורחים: ${e.guests}` : ''].filter(Boolean).join('\n');
      const res = await fetch('/api/youtube/upload-from-drive', { method:'POST', headers:{'content-type':'application/json'}, body: JSON.stringify({ episodeId:e.id, hostId: isHost ? e.ownerHostId : undefined, driveFileId: file.id, title:`#${e.number} · ${e.title}`, description }) });
      const data = await res.json();
      if(!res.ok || !data.ok) throw new Error(data.message || 'ההעלאה נכשלה');
      if(isHost) refreshHostEpisodes();
      else setStore(s=>({...s, episodes:s.episodes.map(x=>x.id===e.id?{...x, youtubeUrl:data.url}:x)}));
      alert(`הפרק הועלה ל-YouTube כסרטון פרטי:\n${data.url}`);
    }catch(err){ alert(err instanceof Error ? err.message : 'ההעלאה ל-YouTube נכשלה'); }
    finally{ setYtUploading(null); }
  }
  const renderRow = (e: Episode, isHost: boolean) => {
    ...
    return <div className={`epRow${isHost ? ' hostRow' : ''}`} key={`${isHost ? e.ownerHostId : 'a'}-${e.id}`} onClick={() => isHost ? openHostEp(e) : router.push(`/episodes/${e.id}`)}>
      ...
      <span onClick={ev => ev.stopPropagation()}><select className="epStatusSel" value={e.status} onChange={ev => isHost ? setHostStatus(e, ev.target.value) : setStatus(e.id, ev.target.value)}>{statusFlow.map(s => <option key={s} value={s}>{s}</option>)}{!statusFlow.includes(e.status) && <option value={e.status}>{e.status}</option>}</select></span>
  ...
  return <>
    ...
    <section className="episodesMobileList">{filtered.map(e=><article className="mobileEpisodeCard" key={e.id}><Link href={`/episodes/${e.id}`} className="cardLink"><div className="mobileEpisodeTop"><span className="pill blue">#{e.number}</span>{e.urgent&&<span className="pill red">דחוף</span>}<span className="pill">{e.status}</span></div><h2>{e.title}</h2><p>{e.topic}</p><div className="mobileEpisodeFacts"><div><small>מנחה</small><b>{e.host}</b></div><div><small>מרואיינים</small><b>{e.guests}</b></div><div><small>צילום</small><b>{cleanDateTime(e.recording)}</b></div><div><small>פרסום</small><b>{cleanDateTime(e.publish)}</b></div></div><div className="progress"><span style={{width:e.progress+'%'}}/></div></Link><div className="mobileEpisodeActions"><button onClick={()=>advance(e.id)}>קדם סטטוס</button><Link href={`/episodes/${e.id}`}>פתח</Link>{e.youtubeUrl ? <a href={e.youtubeUrl} target="_blank" rel="noreferrer">✓ ביוטיוב</a> : <button onClick={()=>uploadEpisodeToYoutube(e,false)} disabled={ytUploading===`a-${e.id}`}>{ytUploading===`a-${e.id}`?'מעלה…':'↑ ליוטיוב'}</button>}<button className="deleteTiny" onClick={()=>deleteEpisode(e.id)} aria-label={`מחיקת ${e.title}`}>מחיקה</button></div></article>)}</section>
    <section className="episodesTableDesktop"><div className="epTable"><div className="epTableHead"><span>מס׳</span><span>פרק</span><span>סטטוס</span><span>מנחה</span><span>מרואיינים</span><span>מועד צילום</span><span>מועד פרסום</span><span>פרק מלא</span><span>סרטוני שיווק</span><span>YouTube</span><span></span></div>{filtered.map(e=>renderRow(e,false))}{hostEpisodes.filter(e=>[e.title,e.topic,e.host,e.guests,e.status,e.ownerName].join(' ').includes(q)).map(e=>renderRow(e,true))}{filtered.length===0&&hostEpisodes.length===0&&<p className="muted" style={{padding:'16px'}}>אין פרקים שתואמים את החיפוש.</p>}</div></section>
...
export function EpisodeDetailClient({ id, initialStore }: { id: string; initialStore?: Store }) {
  ...
  const [socialTextOpen, setSocialTextOpen] = useState(false);
  ...
  async function saveSubtitleDraft(job: MarketingAudioSyncJob) {
    ...
    try {
      ...
      await refreshStoreFromServer();
      setSubtitleSaveNotice(`הטיוטה נשמרה · ${new Date().toLocaleTimeString('he-IL', { hour: '2-digit', minute: '2-digit', second: '2-digit' })}`);
    } catch (error) {
      const message = error instanceof Error ? error.message : 'שמירת הכתוביות נכשלה';
      setSubtitleSaveNotice(message);
  ...
  async function copySocialText() {
    const text = episodeSocialText(ep);
    if (!text) return;
    try {
      await navigator.clipboard.writeText(text);
      setSocialCopied(true);
      window.setTimeout(() => setSocialCopied(false), 1800);
    } catch {
      window.prompt('העתקת טקסט לסושיאל', text);
    }
  }
  ...
  return <>
    ...
    {briefOpen && <Modal title="בריף ותוכן" subtitle="המידע נשמר בתוך מרכז הפרק." onClose={()=>setBriefOpen(false)}><form className="smartForm" onSubmit={saveBrief}><label className="formRow wide"><span>בריף לפרק</span><textarea name="brief" rows={4} defaultValue={ep.brief || ''}/></label><label className="formRow wide"><span>תוכנית תוכן / שאלות</span><textarea name="contentPlan" rows={4} defaultValue={ep.contentPlan || ''}/></label><label className="formRow wide"><span>טקסט לסושיאל</span><textarea name="socialText" rows={4} defaultValue={episodeSocialText(ep)} placeholder="אחרי חיבור סאונד וכתוביות המערכת תמלא כאן פסקה קצרה לפי הכתוביות שזוהו." /></label><label className="formRow wide"><span>תיאום ודגשים</span><textarea name="coordinationNote" rows={4} defaultValue={ep.coordinationNote || ''}/></label><label className="formRow wide"><span>נכסים וחומרים</span><textarea name="assetsNote" rows={4} defaultValue={ep.assetsNote || ''}/></label><div className="formActions"><button className="btn light" type="button" onClick={()=>setBriefOpen(false)}>ביטול</button><button className="btn gold">שמירה</button></div></form></Modal>}
    {socialTextOpen && <Modal title="טקסט לסושיאל" subtitle="סיכום קצר בפורמט שמתאים לשליחה בוואטסאפ." onClose={()=>setSocialTextOpen(false)}><div className="panel subtle"><div style={{display:'flex',justifyContent:'space-between',gap:12,alignItems:'center',marginBottom:12}}><b>טקסט מוכן לשליחה</b><button className="miniBtn" type="button" onClick={copySocialText} disabled={!episodeSocialText(ep)}>{socialCopied ? 'הועתק ✓' : 'העתק הכל'}</button></div><p style={{whiteSpace:'pre-wrap',margin:0,direction:'rtl'}}>{episodeSocialText(ep) || 'עדיין אין טקסט לסושיאל. אחרי חיבור סאונד וכתוביות המערכת תיצור כאן פסקה מסוכמת.'}</p></div><div className="formActions"><button className="btn light" type="button" onClick={()=>setSocialTextOpen(false)}>סגור</button><button className="btn gold" type="button" onClick={()=>{setSocialTextOpen(false); setBriefOpen(true);}}>עריכת הטקסט</button></div></Modal>}
...
function validPodcastDuration(value: string) { return !value || /^\d{1,2}:\d{2}(:\d{2})?$/.test(value.trim()); }
...
function SpotifyPodcastManager({ store, setStore, notice, setNotice }: { store: Store; setStore: React.Dispatch<React.SetStateAction<Store>>; notice: string; setNotice: (value:string)=>void }) {
 ...
 const [publishMode,setPublishMode]=useState<PodcastPublishStatus>('published');
 ...
 function chooseSource(id:string){ const ep=store.episodes.find(e=>String(e.id)===id); if(!ep) return; setSelectedId('new'); setPublishMode('published'); window.setTimeout(()=>{ const titleInput=document.querySelector<HTMLInputElement>('input[name="podcastTitle"]'); if(titleInput) titleInput.value=ep.title; },0); }
 ...
 async function uploadImage(file: File, episodeId?: string){
  setUploading(true); setNotice('');
  addPublishLog(`מעלה ומתקן תמונת פרק ל־Spotify: ${file.name} (${bytesLabel(file.size)})`);
  try{
   if(!file.type.startsWith('image/')) throw new Error('אפשר להעלות רק קובץ תמונה');
   if(file.size > 15*1024*1024) throw new Error('התמונה גדולה מדי — עד 15MB לפני עיבוד');
   const body=new FormData(); body.set('file',file); if(episodeId) body.set('episodeId',episodeId);
   const res=await fetch('/api/podcast/spotify/upload-image',{method:'POST',body}); const data=await res.json();
   if(!res.ok || !data.ok) throw new Error(data.message || 'העלאת תמונה נכשלה');
   addPublishLog('התמונה עלתה בהצלחה ותוקנה אוטומטית ל־3000×3000 JPG', 'success');
   return data;
  }catch(error){
   addPublishLog(error instanceof Error ? error.message : 'העלאת תמונה נכשלה', 'error');
   throw error;
  }finally{ setUploading(false); }
 }
 async function importAudioFromDrive(sourceEpisodeId: number, episodeId: string){
  setUploading(true); setNotice('');
  const ep=store.episodes.find(e=>e.id===sourceEpisodeId);
  addPublishLog(`מחפש את קובץ האודיו הרשמי בדרייב של הפרק${ep ? `: ${ep.title}` : ''}…`);
  try{
   const res=await fetch('/api/podcast/spotify/import-from-drive',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({sourceEpisodeId,podcastEpisodeId:episodeId})});
   const data=await res.json();
   if(!res.ok || !data.ok) throw new Error(data.message || 'ייבוא אודיו מ־Drive נכשל');
   addPublishLog(`האודיו הרשמי יובא מ־Drive והועלה לאחסון הפודקאסט: ${data.audioFileName || data.sourceFileName}`, 'success');
   return data;
  }catch(error){
   addPublishLog(error instanceof Error ? error.message : 'ייבוא אודיו מ־Drive נכשל', 'error');
   throw error;
  }finally{ setUploading(false); }
 }
 function importSocialTextToDescription(){
  const sourceId=Number(document.querySelector<HTMLSelectElement>('select[name="sourceEpisodeId"]')?.value || defaults.sourceEpisodeId) || undefined;
 ...
 async function save(ev: FormEvent<HTMLFormElement>){
  ...
  try{
   ...
   const imageFile=(f.elements.namedItem('imageFile') as HTMLInputElement)?.files?.[0];
   const manualAudioUrl=field(f,'audioUrl');
   const sourceEpisodeId=Number(field(f,'sourceEpisodeId')) || undefined;
   if(!file && !manualAudioUrl && !selected?.audioUrl && !sourceEpisodeId) throw new Error('חסר פרק מקור עם אודיו רשמי, קובץ אודיו או Audio URL ידני');
   if(manualAudioUrl && !validHttpsUrl(manualAudioUrl)) throw new Error('Audio URL חייב להיות קישור https תקין');
   const statusValue=field(f,'podcastStatus') as PodcastPublishStatus;
   const description=field(f,'podcastDescription');
   if((statusValue==='published' || statusValue==='scheduled') && description.trim().length < 10) throw new Error('חסר תיאור פרק מספיק לפרסום ב־Spotify');
   const duration=field(f,'duration');
   if(!validPodcastDuration(duration)) throw new Error('משך הפרק צריך להיות בפורמט 12:34 או 01:12:34');
   if(Number(field(f,'episodeNumber')) < 0 || Number(field(f,'seasonNumber')) < 0) throw new Error('מספר פרק ועונה חייבים להיות חיוביים');
   const scheduledIso=localToIso(field(f,'scheduledAt'));
   if(statusValue==='scheduled' && !scheduledIso) throw new Error('בחר זמן פרסום לתזמון הפרק');
   if(statusValue==='scheduled' && new Date(scheduledIso).getTime() <= Date.now()) throw new Error('מועד תזמון חייב להיות בעתיד');
   addPublishLog('הבדיקות עברו — מכין פרק לשמירה');
   let uploaded: Partial<PodcastEpisode> = {};
   let uploadedImage: { imageUrl?: string } = {};
   const id=selected?.id || crypto.randomUUID();
   if(file) uploaded=await uploadAudio(file,id);
   else if(manualAudioUrl || selected?.audioUrl) addPublishLog('משתמש ב־Audio URL קיים/ידני, מדלג על ייבוא מ־Drive');
   else if(sourceEpisodeId) uploaded=await importAudioFromDrive(sourceEpisodeId,id);
   if(imageFile) uploadedImage=await uploadImage(imageFile,id);
   else if(selected?.imageUrl) addPublishLog('משתמש בתמונת הפרק הקיימת');
   else addPublishLog('לא הועלתה תמונת פרק — Spotify ישתמש בקאבר הראשי של הפודקאסט');
   addPublishLog(statusValue==='published' ? 'נבחר: פרסום עכשיו — הפרק ייכנס מיד ל־RSS' : statusValue==='scheduled' ? 'נבחר: תזמון — הפרק ייכנס ל־RSS בזמן שנבחר' : 'נבחר: טיוטה/ארכיון — הפרק לא יופיע ב־RSS');
   const payload: Partial<PodcastEpisode>={ id, sourceEpisodeId, title, description, episodeNumber:Number(field(f,'episodeNumber')) || undefined, seasonNumber:Number(field(f,'seasonNumber')) || undefined, status:statusValue, scheduledAt:scheduledIso, publishedAt:localToIso(field(f,'publishedAt')) || (statusValue==='published'?new Date().toISOString():undefined), audioUrl:manualAudioUrl, imageUrl:uploadedImage.imageUrl || field(f,'existingImageUrl'), spotifyUrl:field(f,'spotifyUrl'), duration, explicit:field(f,'explicit')==='on', ...uploaded };
   ...
   await refresh();
   addPublishLog(statusValue==='published' ? 'הצלחה: הפרק פורסם ונמצא עכשיו ב־RSS. הוא יופיע ב־Spotify רק אחרי שה־RSS הזה מחובר לעמוד הפודקאסט ב־Spotify Creators וסונכרן שם.' : statusValue==='scheduled' ? 'הצלחה: הפרק נשמר וייכנס ל־RSS בזמן התזמון.' : 'הצלחה: הפרק נשמר, אבל לא יופיע ב־RSS עד שינוי סטטוס ל־Published/מתוזמן.', 'success');
 ...
 async function remove(id:string){
  const ep=episodes.find(e=>e.id===id); if(!ep || !window.confirm(`למחוק את “${ep.title}” מ־Spotify/RSS? הפרק יוסר מהפיד ש־Spotify קורא בסנכרון הבא. קובץ האודיו ב־Supabase לא נמחק אוטומטית.`)) return;
  ...
  setSelectedId('new'); setNotice('הפרק הוסר מה־RSS של Spotify. Spotify יסנכרן את ההסרה בזמן הקרוב.'); await refresh();
 }
 ...
 return <section className="platformWorkspace grid two" aria-label="ניהול Spotify">
  <div className="platformWorkspaceHead panel"><span className={storageOk?'pill green':status?.configured?'pill blue':'pill red'}>Spotify</span><div><h2>ניהול Spotify / RSS עצמאי</h2><p className="muted">פודקש מייצרת RSS Feed תקני ומגישה פרקים מאחסון Supabase נפרד. אחרי שמחברים את ה־RSS הזה לעמוד הנכון ב־Spotify Creators, כל פרק Published/מתוזמן יופיע שם אחרי סנכרון של Spotify.</p></div></div>
  <div className="platformWorkspaceBody grid two">
   <div className="panel dark"><h2>Feed וחיבור</h2><div className="list"><div className="row"><span>סטטוס Supabase</span><b>{status?.message || 'בודק…'}</b></div><div className="row"><span>עמוד Spotify יעד</span><b>{status?.spotifyShowId || '033eNDxQDdcRftOLpRmv29'}</b></div><div className="row"><span>Bucket</span><b>{status?.bucket || 'podcast-audio'}</b></div><div className="row"><span>RSS לחיבור ב־Spotify Creators</span><code className="inlineCode">{status?.feedUrl || '/api/podcast/spotify/rss'}</code></div><div className="row"><span>פרקים ב־RSS</span><b>{status?.publishedCount ?? 0} מתוך {status?.totalCount ?? episodes.length}</b></div></div><div className="formActions" style={{marginTop:14}}><button className="btn light" type="button" onClick={()=>refresh()}>רענון</button>{status?.feedUrl?<a className="btn gold" href={status.feedUrl} target="_blank" rel="noreferrer">פתיחת RSS</a>:null}{status?.spotifyShowUrl?<a className="btn light" href={status.spotifyShowUrl} target="_blank" rel="noreferrer">פתיחת עמוד היעד בספוטיפיי</a>:null}</div>{!storageOk?<p className="muted" style={{marginTop:14}}>כדי להעלות אודיו צריך ליצור Supabase project נפרד, bucket ציבורי בשם <b>podcast-audio</b>, ולהוסיף env vars ב־Vercel. עד אז אפשר לנהל metadata ולהדביק Audio URL חיצוני ידנית.</p>:null}</div>
   <div className="panel"><h2>פרקים מנוהלים</h2><div className="list">{episodes.length?episodes.map(ep=><div className="row" key={ep.id}><span><b>{ep.title}</b><br/><small className="muted">{bytesLabel(ep.audioBytes)} · {ep.scheduledAt ? cleanDateTime(ep.scheduledAt) : cleanDateTime(ep.publishedAt || ep.createdAt)}</small></span><span style={{display:'inline-flex',gap:8,alignItems:'center',flexWrap:'wrap',justifyContent:'flex-end'}}><span className={podcastStatusClass(ep.status)}>{podcastStatusLabels[ep.status]}</span><button className="miniBtn" type="button" onClick={()=>setSelectedId(ep.id)}>עריכה</button><button className="miniBtn" type="button" onClick={()=>remove(ep.id)}>מחיקה</button></span></div>):<p className="muted">עדיין אין פרקים ב־Spotify. צור פרק ראשון מהטופס.</p>}</div><button className="btn dark" style={{marginTop:14}} type="button" onClick={()=>setSelectedId('new')}>+ פרק Spotify חדש</button></div>
   <div className="panel" style={{gridColumn:'1 / -1'}}><div style={{display:'flex',justifyContent:'space-between',gap:12,alignItems:'start',flexWrap:'wrap'}}><div><h2>פרקים פעילים עכשיו בספוטיפיי</h2><p className="muted" style={{marginTop:4}}>אלה הפרקים שנמצאים כרגע בפיד הפעיל ש־Spotify קורא: Published עם אודיו, או מתוזמנים שמועד הפרסום שלהם כבר הגיע.</p></div><span className="pill green">{liveEpisodes.length} פעילים</span></div><div className="list" style={{marginTop:12}}>{liveEpisodes.length?liveEpisodes.map(ep=><div className="row" key={`live-${ep.id}`}><span><b>{ep.title}</b><br/><small className="muted">פרק {ep.episodeNumber || '—'} · {cleanDateTime(ep.publishedAt || ep.scheduledAt || ep.createdAt)}{ep.spotifyUrl?' · יש קישור Spotify':' · ממתין/בלי קישור Spotify שמור'}</small></span><span style={{display:'inline-flex',gap:8,alignItems:'center',flexWrap:'wrap',justifyContent:'flex-end'}}>{ep.spotifyUrl?<a className="miniBtn" href={ep.spotifyUrl} target="_blank" rel="noreferrer">פתח בספוטיפיי</a>:null}<button className="miniBtn" type="button" onClick={()=>setSelectedId(ep.id)}>עריכה</button><button className="miniBtn" type="button" onClick={()=>remove(ep.id)}>מחיקה מספוטיפיי</button></span></div>):<p className="muted">אין כרגע פרקים פעילים בפיד Spotify. פרקים יופיעו כאן אחרי שמירתם כ־Published עם אודיו תקין.</p>}</div></div>
   <div className="panel" style={{gridColumn:'1 / -1'}}><h2>{selected?'עריכת פרק Spotify':'יצירת פרק Spotify'}</h2><form className="smartForm" onSubmit={save} key={selected?.id || 'new'}><FormRow label="פרק מקור בפודקש"><select name="sourceEpisodeId" defaultValue={String(defaults.sourceEpisodeId)} onChange={e=>chooseSource(e.target.value)}>{store.episodes.map(e=><option key={e.id} value={e.id}>{`#${e.number} · ${e.title}`}</option>)}</select></FormRow><FormRow label="כותרת"><input name="podcastTitle" defaultValue={defaults.title} required /></FormRow><FormRow label="מה לעשות עם הפרק"><select name="podcastStatus" value={publishMode} onChange={e=>setPublishMode(e.target.value as PodcastPublishStatus)}><option value="published">פרסם עכשיו</option><option value="scheduled">תזמן לפרסום</option><option value="draft">שמור כטיוטה</option><option value="archived">ארכיון / מוסתר</option></select></FormRow>{publishMode==='scheduled'?<FormRow label="מועד פרסום"><input name="scheduledAt" type="datetime-local" defaultValue={defaults.scheduledAt || nowLocalInput()} /></FormRow>:null}{publishMode==='published'?<FormRow label="פורסם בפועל"><input name="publishedAt" type="datetime-local" defaultValue={defaults.publishedAt} placeholder="ריק = עכשיו" /></FormRow>:null}<FormRow label="מספר פרק"><input name="episodeNumber" type="number" defaultValue={defaults.episodeNumber} /></FormRow><FormRow label="עונה"><input name="seasonNumber" type="number" defaultValue={defaults.seasonNumber} /></FormRow><FormRow label="משך"><input name="duration" placeholder="01:12:30" defaultValue={defaults.duration} /></FormRow><FormRow label="קובץ אודיו ידני (אופציונלי)"><input name="audioFile" type="file" accept="audio/*" disabled={!storageOk || uploading} /></FormRow><FormRow label="Audio URL ידני (אופציונלי)"><input name="audioUrl" type="url" defaultValue={defaults.audioUrl} placeholder="ברירת מחדל: ייבוא אוטומטי מתיקיית האודיו הרשמי ב־Drive" /></FormRow><FormRow label="תמונת פרק (קובץ)"><input name="imageFile" type="file" accept="image/jpeg,image/png,image/webp" disabled={!storageOk || uploading} /><input name="existingImageUrl" type="hidden" defaultValue={defaults.imageUrl} />{defaults.imageUrl?<small className="muted">יש תמונה קיימת לפרק. העלאה חדשה תחליף אותה.</small>:<small className="muted">אופציונלי — אם לא מעלים תמונה, Spotify ישתמש בקאבר הראשי. המערכת ממירה אוטומטית ל־JPG מרובע 3000×3000.</small>}</FormRow><FormRow label="קישור Spotify אחרי קליטה"><input name="spotifyUrl" type="url" defaultValue={defaults.spotifyUrl} placeholder="אופציונלי — למעקב אחרי שספוטיפיי קלט" /></FormRow><label className="checkRow"><input type="checkbox" name="explicit" defaultChecked={defaults.explicit}/> תוכן Explicit</label><label className="formRow wide"><span style={{display:'flex',justifyContent:'space-between',gap:12,alignItems:'center',flexWrap:'wrap'}}>תיאור הפרק <small className="muted">חובה לפרסום/תזמון</small><button className="miniBtn" type="button" onClick={importSocialTextToDescription}>ייבוא טקסט לסושיאל</button></span><textarea name="podcastDescription" rows={7} defaultValue={defaults.description} placeholder="תקציר ברור של הפרק, שמות אורחים, נושאים מרכזיים וכל מידע שיעזור לספוטיפיי ולאנשים להבין מה יש בפרק." /></label><div className="panel subtle" style={{gridColumn:'1 / -1'}}><b>בדיקות לפני פרסום</b><p className="muted" style={{margin:'6px 0 0'}}>לפרסום תקין צריך כותרת, תיאור, אודיו זמין ותזמון עתידי אם בחרת לתזמן. תמונת פרק היא אופציונלית — אבל אם מעלים, היא תעובד אוטומטית לפורמט שמתאים ל־Spotify.</p></div><div className="formActions"><button className="btn light" type="button" onClick={()=>setSelectedId('new')}>נקה</button>{selected?<button className="btn light" type="button" onClick={()=>remove(selected.id)}>מחיקה</button>:null}<button className="btn gold" disabled={saving || uploading}>{uploading?'מעלה אודיו…':saving?'שומר…':'שמירת פרק Spotify'}</button></div></form></div>
