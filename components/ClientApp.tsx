...
export function EpisodesClient() {
  ...
  return <>
    ...
    <section className="episodesTableDesktop"><div className="epTable"><div className="epTableHead"><span>מס׳</span><span>פרק</span><span>סטטוס</span><span>מנחה</span><span>מרואיינים</span><span>מועד צילום</span><span>פרק מלא</span><span>סרטוני שיווק</span><span>התקדמות</span><span>משימות</span><span></span></div>{filtered.map(e=>{const fv=e.driveAssetStatus?.fullVideo;const fullFolder=e.fullVideoFolderUrl||e.fullVideoUrl;const fullFile=fv?.files?.[0]?.url||fullFolder;const mk=e.driveMarketingFolderUrl||e.shortsDriveFolderUrl;const mkHas=e.driveAssetStatus?.marketing?.hasFiles;return <div className="epRow" key={e.id} onClick={()=>router.push(`/episodes/${e.id}`)}><span className="epNum">#{e.number}</span><span className="epTitle"><b>{e.title}</b><small>{e.topic}</small></span><span onClick={ev=>ev.stopPropagation()}><select className="epStatusSel" value={e.status} onChange={ev=>setStatus(e.id, ev.target.value)}>{statusFlow.map(s=><option key={s} value={s}>{s}</option>)}{!statusFlow.includes(e.status)&&<option value={e.status}>{e.status}</option>}</select></span><span title={e.host}>{e.host}</span><span title={e.guests}>{e.guests}</span><span className="epDate">{cleanDateTime(e.recording)}</span><span className="epAssetCell" onClick={ev=>ev.stopPropagation()}>{fv?.hasFiles&&fullFile?<a className="epAsset done" href={fullFile} target="_blank" rel="noreferrer">✓ צפה בפרק</a>:fullFolder?<a className="epAsset todo" href={fullFolder} target="_blank" rel="noreferrer">↑ העלה</a>:<button className="epAsset sync" type="button" onClick={()=>syncEpisodeDrive(e.id)}>צור תיקייה</button>}</span><span className="epAssetCell" onClick={ev=>ev.stopPropagation()}>{mk?<a className={`epAsset ${mkHas?'done':'todo'}`} href={mk} target="_blank" rel="noreferrer">{mkHas?'✓ סרטונים':'תיקיית סרטונים'}</a>:<button className="epAsset sync" type="button" onClick={()=>syncEpisodeDrive(e.id)}>צור תיקייה</button>}</span><span className="epProg"><span className="progress"><span style={{width:e.progress+'%'}}/></span><small>{e.progress}%</small></span><span className="epTasks">{taskCount(e)}</span><span onClick={ev=>ev.stopPropagation()}><button className="deleteTiny" onClick={()=>deleteEpisode(e.id)} aria-label={`מחיקת ${e.title}`}>מחק</button></span></div>;})}{filtered.length===0&&<p className="muted" style={{padding:'16px'}}>אין פרקים שתואמים את החיפוש.</p>}</div></section>
