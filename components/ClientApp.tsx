...
export function EpisodesClient() {
  ...
  return <>
    ...
    <section className="episodesTableDesktop"><div className="epTable"><div className="epTableHead"><span>מס׳</span><span>פרק</span><span>סטטוס</span><span>מנחה</span><span>מרואיינים</span><span>מועד צילום</span><span>מועד פרסום</span><span>התקדמות</span><span>משימות</span><span></span></div>{filtered.map(e=><div className="epRow" key={e.id} onClick={()=>router.push(`/episodes/${e.id}`)}><span className="epNum">#{e.number}</span><span className="epTitle"><b>{e.title}</b><small>{e.topic}</small></span><span onClick={ev=>ev.stopPropagation()}><select className="epStatusSel" value={e.status} onChange={ev=>setStatus(e.id, ev.target.value)}>{statusFlow.map(s=><option key={s} value={s}>{s}</option>)}{!statusFlow.includes(e.status)&&<option value={e.status}>{e.status}</option>}</select></span><span title={e.host}>{e.host}</span><span title={e.guests}>{e.guests}</span><span className="epDate">{cleanDateTime(e.recording)}</span><span className="epDate">{cleanDateTime(e.publish)}</span><span className="epProg"><span className="progress"><span style={{width:e.progress+'%'}}/></span><small>{e.progress}%</small></span><span className="epTasks">{taskCount(e)}</span><span onClick={ev=>ev.stopPropagation()}><button className="deleteTiny" onClick={()=>deleteEpisode(e.id)} aria-label={`מחיקת ${e.title}`}>מחק</button></span></div>)}{filtered.length===0&&<p className="muted" style={{padding:'16px'}}>אין פרקים שתואמים את החיפוש.</p>}</div></section>
