...
function segmentsToSrt(segments){ return segments.slice().sort((a,b)=>a.index-b.index).map((seg,i)=>`${i+1}\n${msToSrtTime(seg.startMs)} --> ${msToSrtTime(Math.max(seg.endMs,seg.startMs+500))}\n${splitCaptionLine(seg.text||'')}`).join('\n\n')+'\n'; }
